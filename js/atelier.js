...
function packExemple() {
  ...
  p.decouvertes.push(d); p.guide.push(g); p.balises.push(b);
  return p;
}
...
document.getElementById("btn-new").addEventListener("click", () => {
  AT.pack = nouveauPack(); AT.decouvertes = []; AT.guide = []; AT.balises = [];
...
document.getElementById("btn-exemple").addEventListener("click", () => {
  ...
  AT.pack = ex.pack; AT.decouvertes = ex.decouvertes; AT.guide = ex.guide; AT.balises = ex.balises;
...
document.getElementById("btn-export").addEventListener("click", () => {
  ...
  const bundle = { $format: "jdpbc-pack", $version: 1, pack: AT.pack, decouvertes: AT.decouvertes, guide: AT.guide, balises: AT.balises };
...
document.getElementById("import-file").addEventListener("change", async (ev) => {
  ...
  try {
    ...
    AT.pack = bundle.pack; AT.decouvertes = bundle.decouvertes || []; AT.guide = bundle.guide || []; AT.balises = bundle.balises || [];
    render();
    const r = validerPack(AT);
  ...
  if (/^[\w./-]+\.json$/.test(h)) {
    try {
      ...
      if (b.$format === "jdpbc-pack") {
        AT.pack = b.pack; AT.decouvertes = b.decouvertes || []; AT.guide = b.guide || []; AT.balises = b.balises || [];
        render();
